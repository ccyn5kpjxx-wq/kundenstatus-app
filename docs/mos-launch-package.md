...
- Kunden sehen Vermieter, Mietdaten, Bruttopreis, 500 € Kaution, 1.000 € vertragliche Selbstbeteiligung und die vollständigen Bedingungen **vor der Zahlung**. Sie zeichnen dort mit Finger oder Maus ihre Unterschrift. Nur ein unverändert gespeicherter, unterschriebener Vertragssnapshot darf zum Checkout weitergeleitet werden; ohne Unterschrift wird keine Reservierung angelegt. Signaturgeprüfter Webhook plus erneute Providerabfrage bestätigt genau einen Mietvorgang. Rücksprung, doppelte Klicks und doppelte Events ersetzen keine Zahlungsprüfung. Nach bestätigter Zahlung wird die unterschriebene Fassung einmalig als PDF gespeichert und steht Kunden sowie der Werkstatt zum Download bereit. Die gezeichnete Unterschrift ist keine qualifizierte elektronische Signatur. Es gibt keinen automatischen E-Mail-Versand.
...
| Stripe / Hosting | Stripe-Live-Konto ist im Browser angemeldet; Karten und Wallets werden angezeigt. Ein nutzbarer Server-Key und Webhook sind nicht eingerichtet. Render-Verwaltung ist nicht angemeldet; die öffentliche `/mieten/`-Route antwortet derzeit mit 404. Echte Sandbox- und Live-Abnahme fehlen. Kein echtes Checkout-/Erstattungsereignis ausgeführt. |
...
5. Der Nutzer hat die Aktivierung echter Zahlung am 23.09.2026 ausdrücklich beauftragt. Den Live-Key erst anlegen, wenn Render-Secretstore und die vollständige, belegte Livekonfiguration als Ziel bereitstehen; HTTPS-Endpunkt und Webhook-Signaturprüfung danach verifizieren. Öffentliche Buttons erst danach über `MOS_PUBLIC_LIVE_ENTRY_URL=https://kundenstatus-app.onrender.com/mieten/` im schlanken MOS-Frontend einblenden. `enabled` und `live_enabled` bleiben bis zur Abnahme false.
